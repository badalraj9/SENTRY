...
// Mock Data (Replace with useGetFeedQuery())
const MOCK_ACTIVITIES: Activity[] = [
  {
    ...
    content: 'The platform will be down for 10m tonight for the Rust migration.',
  ...
  },
  {
    ...
    type: 'DECISION',
    user: 'Alex Chen',
    title: 'Decision Approved: Move to Vite',
    content: 'The team has voted to replace Webpack with Vite/Rspack.',
    timestamp: new Date(Date.now() - 1000 * 60 * 60 * 2),
    metadata: { status: 'approved', decisionId: '104' },
  },
  {
    id: '3',
    type: 'WORKSHOP',
    user: 'Sarah Jones',
    title: 'Workshop Completed: Q3 Roadmap',
    content: '32 action items generated. Click to view summary.',
    timestamp: new Date(Date.now() - 1000 * 60 * 60 * 24),
    metadata: { actionItems: 32, workshopId: 'ws-001' },
  },
  {
    id: '4',
    type: 'DOCUMENT',
    user: 'Jordan Lee',
    title: 'New Document: API Specification v2',
    content: 'Published updated API documentation for the new endpoints.',
    timestamp: new Date(Date.now() - 1000 * 60 * 60 * 48),
    metadata: { docId: 'doc-123' },
  },
  {
    id: '5',
    type: 'DECISION',
    user: 'Morgan Smith',
    title: 'Decision Pending: Database Migration',
    content: 'Proposal to migrate from Postgres to CockroachDB.',
    timestamp: new Date(Date.now() - 1000 * 60 * 60 * 72),
    metadata: { status: 'pending', decisionId: '105' },
  },
];
    ...
    <div className={cn(
      ...
      <div className="flex-1 space-y-1 min-w-0">
        <div className="flex justify-between items-start gap-2">
          <h3 className="text-sm font-medium text-terminal-200 truncate">{title}</h3>
          <span className="text-xs font-mono text-terminal-600 flex-shrink-0">{time}</span>
        </div>
  ...
  return (
    <div className="h-full flex flex-col bg-terminal-950">
      ...
      <div className="flex-1 overflow-y-auto">
        {activities.length === 0 ? (
          <div className="flex items-center justify-center h-32 text-terminal-500 font-mono text-sm">
            No activities found.
          </div>
        ) : (
          activities.map((item) => {
            ...
            switch (item.type) {
              ...
              case 'DECISION':
                ...
                return (
                  ...
                  >
                    <p>
                      Proposal by <span className="text-terminal-300">{item.user}</span>.
                      Status:{' '}
                      <span className={cn(
              ...
              case 'DOCUMENT':
                return (
                  ...
                  >
                    <p>
                      Published by <span className="text-terminal-300">{item.user}</span>.{' '}
                      {item.content}
                    </p>
                  </FeedCard>
                );
