  ...
  io.on('connection', (socket) => {
    ...
    socket.on('subscribe', (channels: string[]) => {
      for (const channel of channels) {
        // Validate channel access (simplified for now)
        socket.join(channel);
      }
    });
  ...
  try {
    await subscribe('sentry:events', (event: unknown) => {
      if (typeof event === 'object' && event !== null) {
        const e = event as { type: WSEventType; room: string; payload: unknown };
        broadcastEvent(io, e);
      }
    });
    console.log('📡 Subscribed to Redis pub/sub channel');
  } catch (error) {
    console.log('⚠️  Redis pub/sub setup failed - WebSocket events are local only');
  }
}
